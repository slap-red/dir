#!/usr/bin/env python3
"""
Simple Flask web application for the Slap Red Scraper.
This version works independently of the complex scraper modules.
"""
...
<body>
    ...
    <div class="alert alert-info">
        <strong>Note:</strong> This is a simplified web interface. The scraper will run using the command-line version with your uploaded URLs.
    </div>
        ...
        def run():
            try:
                ...
                result = subprocess.run(['python', 'main.py'], 
                                      capture_output=True, text=True, cwd='/workspace/dir')
                
                if result.returncode == 0:
                    scraper_status.update({"message": "Completed", "progress": len(urls)})
                else:
                    scraper_status.update({"message": f"Error: {result.stderr}", "progress": 0})
